...
enum GitHubError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidURL(let path):
            return "Invalid GitHub API URL: \(path)"
        case .invalidResponse:
            return "Invalid response from GitHub API"
        case .unauthorized(let message):
            return "GitHub unauthorized: \(message)"
        case .notFound(let resource, let message):
            return "GitHub resource not found (\(resource)): \(message)"
        case .conflict(let message):
            return "GitHub conflict: \(message)"
        case .validationError(let message):
            return "GitHub validation error: \(message)"
        case .forbidden(let message):
            return "GitHub forbidden: \(message)"
        case .apiError(let statusCode, let message):
            return "GitHub API error (\(statusCode)): \(message)"
        case .decodingFailed(let path):
            return "Failed to decode file content: \(path)"
        case .encodingFailed(let path):
            return "Failed to encode file content: \(path)"
        }
    }
}
